...
| Windows input selection | Clipboard then paste; Enter command | `windows-server/test_backend.py` | Existing automated |
...
| Windows install | Run entry starts receiver; firewall/private network and SendKeys work | Guarded live on Windows |
...
1. **High:** Ubuntu Wayland and Windows have not yet been exercised on physical target machines; command construction is tested but OS policy can still block input.
